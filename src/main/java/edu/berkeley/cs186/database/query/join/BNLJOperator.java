...
public class BNLJOperator extends JoinOperator {
    ...
    private class BNLJIterator implements Iterator<Record>{
        ...
        private Record fetchNextRecord() {
            ...
            while(true) {
                if (rightPageIterator.hasNext()) {
                    ...
                    // Case 2:
                    // The right page iterator doesn't have a value to
                    // yield but the left block iterator does
                    leftRecord = leftBlockIterator.next();
                    resetRightPageRecord();
                    //fetchNextRightPage();
                } else if (rightSourceIterator.hasNext()){
                    // Case 3:
                    // Neither the right page nor left block iterators
                    // have values to yield, but there's more
                    // right pages
                    fetchNextRightPage();
                    resetLeftPageRecord();
                } else if (leftSourceIterator.hasNext()) {
                    // Case 4:
                    // Neither right page nor left block iterators
                    // have values nor are there more right
                    //pages, but there are still left blocks
                    fetchNextLeftBlock();
        ...
        private void resetLeftPageRecord() {
            this.leftBlockIterator.reset();
            this.leftRecord = leftBlockIterator.next();
        }

        private void resetRightPageRecord() {
            this.rightPageIterator.reset();
            // assert(rightPageIterator.hasNext() == true);
            if (rightPageIterator.hasNext())
