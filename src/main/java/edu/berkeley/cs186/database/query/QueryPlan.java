...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        // You should first calculate the estimated I/O cost of a sequential scan, since this is always
        // possible (it's the default option: we only move away from it in favor of index scans if the index
        // scan is both possible and more efficient).
        QueryOperator seqScan = new SequentialScanOperator(this.transaction, table);
        int seqScanCost = seqScan.estimateIOCost();

        // Then, if there are any indices on any column of the table that we have a selection predicate on,
        // you should calculate the estimated I/O cost of doing an index scan on that column. If any of
        // these are more efficient than the sequential scan, take the best one.
        List<Integer> indexes = getEligibleIndexColumns(table);
        int lowestCost = seqScanCost;
        int bestInd = -1;
        for (int index : indexes) {
            ...
            if (lowestCost > indScanCost) {
                lowestCost = indScanCost;
                bestInd = index;
            }
        }
        // Finally, as part of a heuristic-based optimization covered in class, you should push down any
        // selection predicates that involve solely the table (see QueryPlan#addEligibleSelections ).
        QueryOperator minOp = addEligibleSelections(seqScan, -1);
        if (bestInd != -1) {
            SelectPredicate predicate = this.selectPredicates.get(bestInd);
            QueryOperator bestIndexScan = new IndexScanOperator(this.transaction, table, predicate.column, predicate.operator, predicate.value);
            minOp = addEligibleSelections(bestIndexScan, bestInd);
        }

        // This should leave you with a query operator beginning with a sequential or index scan operator,
        // followed by zero or more SelectOperator s.
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> tables: prevMap.keySet()) {
            ...
            for (JoinPredicate predicates: this.joinPredicates) {

                // Get the left side and the right side of the predicate (table name and column)
                String rightTableName = predicates.rightTable;
                String rightCol = predicates.rightColumn;
                String leftTableName = predicates.leftTable;
                String leftCol = predicates.leftColumn;

                Set<String> pass1TableSet = null;
                ...
                if (tables.contains(leftTableName) && !tables.contains(rightTableName)) {
                    ...
                    pass1TableSet = getTableSet(pass1Map, rightTableName);
                    ...
                    joinOp = minCostJoinType(prevMap.get(tables), pass1Map.get(pass1TableSet), leftCol, rightCol);
                    ...
                    pass1TableSet = getTableSet(pass1Map, leftTableName);

                    // Using the operator from Case 1 or 2, use minCostJoinType to
                    // calculate the cheapest join with the new table (the one you
                    // fetched an operator for from pass1Map) and the previously joined
                    // tables. Then, update the result map if needed.
                    joinOp = minCostJoinType(prevMap.get(tables), pass1Map.get(pass1TableSet), rightCol, leftCol);
                } else {
                    // Case 3: Otherwise, skip this join predicate and continue the loop.
                    continue;
                }

                // Combined the set from the new table and the previously joined table.
                Set<String> combinedSet = new HashSet<String>();
                combinedSet.addAll(tables);
                combinedSet.addAll(pass1TableSet);
                ...
                if (result.containsKey(combinedSet)) {
                    currentCost = result.get(combinedSet).estimateIOCost();
                }
                int prevCost = Integer.MAX_VALUE;
                if (prevMap.containsKey(combinedSet)) {
                    prevCost = prevMap.get(combinedSet).estimateIOCost();
                }
                if(prevCost > cost && currentCost > cost) {
                    result.put(combinedSet, joinOp);
                }
            }
        }
        return result;
    ...
    private Set<String> getTableSet(Map<Set<String>, QueryOperator> pass1Map, String tableName) {
        for (Set<String> tables : pass1Map.keySet()) {
    ...
    public Iterator<Record> execute() {
        ...
        for(String table: this.tableNames) {
            QueryOperator ops = minCostSingleAccess(table);
            Set<String> tables = new HashSet<>();
            tables.add(table);
            pass1Map.put(tables, ops);
        }
