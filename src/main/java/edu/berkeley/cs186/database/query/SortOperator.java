...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        List<Iterator<Record>> iter = new ArrayList();
        for (int i = 0; i < runs.size(); i++) {
            Iterator<Record> r = runs.get(i).iterator();
            iter.add(i, r);
            while (iter.get(i).hasNext())
                pq.add(new Pair<> (iter.get(i).next(), i));
        }
        while (!pq.isEmpty()) {
    ...
    public List<Run> mergePass(List<Run> runs) {
        // TODO(proj3_part1): implement
        List<Run> rtnRuns = new ArrayList<>();
        for (int i = 0; i < runs.size(); i += numBuffers - 1) {
            int end;
            if (runs.size() < i + numBuffers -1)
                end = runs.size();
            else
                end = i + numBuffers -1;
            rtnRuns.add(mergeSortedRuns(runs.subList(i, end)));
        }
        return rtnRuns;
    }
    ...
    public Run sort() {
        ...
        BacktrackingIterator<Record> recordIter = getBlockIterator(sourceIterator, getSchema(), numBuffers);
        sorted.add(sortRun(recordIter));
        while (sorted.size() > 1) {
