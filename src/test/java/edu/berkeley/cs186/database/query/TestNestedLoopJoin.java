...
public class TestNestedLoopJoin {
    ...
    public void testSimpleJoinBNLJ() {
        ...
        try(Transaction transaction = d.beginTransaction()) {
            ...
            while (outputIterator.hasNext() && numRecords < 100 * 100) {
                ...
                assertEquals("mismatch at record " + numRecords, expectedRecord, test);
                numRecords++;
            }
            checkIOs(0);
    ...
    @Test
    @Category(PublicTests.class)
    public void testNonEmptyWithEmptyJoinBNLJ() {
        // This test is identical to the above test, but uses your BNLJ
        // with B=5 instead.
        d.setWorkMem(5); // B=5
        try(Transaction transaction = d.beginTransaction()) {
            ...
            while (outputIterator.hasNext() && numRecords < 100 * 0) {
                Record test = outputIterator.next();
                assertEquals("mismatch at record " + numRecords, expectedRecord, test);
                numRecords++;
            }
            ...
            assertFalse("too many records", outputIterator.hasNext());
            assertEquals("too few records", 100 * 0, numRecords);
        }
    }

    @Test
    @Category(PublicTests.class)
    public void testEmptyWithNonEmptyJoinBNLJ() {
        // This test is identical to the above test, but uses your BNLJ
        // with B=5 instead.
        d.setWorkMem(5); // B=5
        try(Transaction transaction = d.beginTransaction()) {
            ...
            while (outputIterator.hasNext() && numRecords < 0 * 100) {
                Record test = outputIterator.next();
                assertEquals("mismatch at record " + numRecords, expectedRecord, test);
                numRecords++;
            }
            ...
            assertFalse("too many records", outputIterator.hasNext());
            assertEquals("too few records", 0 * 100, numRecords);
        }
    }

    @Test
    @Category(PublicTests.class)
    public void testEmptyWithEmptyJoinBNLJ() {
        // This test is identical to the above test, but uses your BNLJ
        // with B=5 instead.
        d.setWorkMem(5); // B=5
        try(Transaction transaction = d.beginTransaction()) {
            ...
            System.out.println("entering while loop");
            while (outputIterator.hasNext() && numRecords < 0 * 0) {
                Record test = outputIterator.next();
                assertEquals("mismatch at record " + numRecords, expectedRecord, test);
                numRecords++;
            }
            ...
            assertFalse("too many records", outputIterator.hasNext());
            assertEquals("too few records", 0 * 0, numRecords);
        }
    }
}
